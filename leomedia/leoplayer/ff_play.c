 ...
 *
 * This file is part of leoplayer.
 *
 * leoplayer is free software; you can redistribute it and/or
...
static int audio_open(void *opaque, int64_t wanted_channel_layout, int wanted_nb_channels, int wanted_sample_rate, struct AudioParams *audio_hw_params)
{
    ...
    if (spec.format != AUDIO_S16SYS) {
        av_log(NULL, AV_LOG_ERROR,
               "SDL advised audio format %d is not is the not default AUDIO_S16SYS!\n", spec.format);
//        return -1;
    }
    ...
    if(AUDIO_S32SYS==spec.format){
        audio_hw_params->fmt = AV_SAMPLE_FMT_S32;
    }else if(AUDIO_F32SYS==spec.format){
...
//           refresh_loop_wait_event(cur_stream, &event);
        ...
        switch (event.type) {
            ...
            case FF_PAUSE_EVENT:
                ...
                ScheduleRefresh_ffplay(cur_stream, 1);
                break;
            case FF_STOP_EVENT:
                av_log(NULL, AV_LOG_ERROR, "FF_STOP_EVENT");
                if (!isStop)
                {
                    do_exit(cur_stream);
                    isStop = 1;
            ...
            case FF_SEEKTO_EVENT:
                seekpos = *(double *)event.user.data1;
                if (seekpos > 0) {
                    pos = get_master_clock(cur_stream);
                    if (isnan(pos))
                        pos = (double)cur_stream->seek_pos / AV_TIME_BASE;
                    incr =seekpos- pos;
                    stream_seek(cur_stream, (int64_t)(seekpos * AV_TIME_BASE), (int64_t)(incr * AV_TIME_BASE),0);
                }
                break;
            default:
                break;
...
void  ffp_global_set_inject_callback(leo_inject_callback cb)
{
...
static const char *leo_ffplay_version_info()
{
    return FFPLAY_VERSION;
}



//start export the method by leoplayer
FFPlayer *ffp_create()
{
    av_log(NULL, AV_LOG_INFO, "av_version_info: %s\n", av_version_info());
    av_log(NULL, AV_LOG_INFO, "leo_version_info: %s\n", leo_ffplay_version_info());
